...
if [ ! -f "config/lint/pre-commit-config.yaml" ]; then
    echo -e "${RED}Error: config/lint/pre-commit-config.yaml not found. This script must be run from the project root directory.${NC}"
    exit 1
fi
...
echo -e "Installing pre-commit hooks defined in config/lint/pre-commit-config.yaml..."
pre-commit install -c config/lint/pre-commit-config.yaml
...
echo -e "To fix code quality issues automatically, run: ${YELLOW}./fix-code-quality.sh${NC}"
