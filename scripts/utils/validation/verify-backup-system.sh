...
test_git_backup() {
    ...
    # Create a backup
    if ./scripts/utils/backup/create-backup-branch.sh; then
        ...
        # Restore from backup
        if ./scripts/utils/backup/restore-from-backup.sh "$BACKUP_BRANCH" test_backup_file.txt; then
...
check_file_exists "scripts/utils/backup/create-backup-branch.sh" || VERIFICATION_FAILED=true
check_file_exists "scripts/utils/backup/restore-from-backup.sh" || VERIFICATION_FAILED=true
...
if [ -f "docs/backup-functionality.md" ]; then
    ...
    check_string_in_file "docs/backup-functionality.md" "cleanup-backup-dirs.sh" || VERIFICATION_FAILED=true
else
