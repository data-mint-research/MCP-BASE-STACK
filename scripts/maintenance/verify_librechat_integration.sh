...
docker exec librechat ls -la /app/librechat.yaml 2>/dev/null | tee -a librechat-model-integration.txt
echo "" | tee -a librechat-model-integration.txt

# View the actual config to verify model settings
echo "=== Examining model configuration ===" | tee -a librechat-model-integration.txt
docker exec librechat cat /app/librechat.yaml 2>/dev/null | tee -a librechat-model-integration.txt
echo "" | tee -a librechat-model-integration.txt
...
docker logs librechat 2>&1 | grep -i "custom" | head -n 10 | tee -a librechat-model-integration.txt
echo "" | tee -a librechat-model-integration.txt

# Verify Mistral direct access configuration
echo "=== Verifying Mistral configuration ===" | tee -a librechat-model-integration.txt
docker exec librechat env | grep -E "MISTRAL" | tee -a librechat-model-integration.txt
echo "" | tee -a librechat-model-integration.txt
...
echo "" | tee -a librechat-model-integration.txt
curl -s http://localhost:8000/debug/llm-server-status | tee -a librechat-model-integration.txt
echo "" | tee -a librechat-model-integration.txt

# Test connection from LibreChat to LLM Server (for Mistral)
echo "=== Testing LibreChat to LLM Server connection (for Mistral) ===" | tee -a librechat-model-integration.txt
if docker exec librechat curl -s http://llm-server:11434/api/tags 2>/dev/null | grep -q "models"; then
  echo "LibreChat can successfully connect to LLM Server" | tee -a librechat-model-integration.txt
else
  echo "LibreChat cannot connect to LLM Server" | tee -a librechat-model-integration.txt
fi
echo "" | tee -a librechat-model-integration.txt
...
if docker exec librechat curl -s http://mcp-server:8000/debug/health 2>/dev/null | grep -q "healthy"; then
...
echo "1. Mistral: Direct integration via LLM Server" | tee -a librechat-model-integration.txt
echo "2. DeepSeek Coder: Accessed through MCP Server, which connects to LLM Server" | tee -a librechat-model-integration.txt
echo "" | tee -a librechat-model-integration.txt
