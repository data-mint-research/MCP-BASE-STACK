...
chmod +x scripts/utils/cleanup/remove_local_backup_system.sh
chmod +x data_sources/knowledge_graph/scripts/remove_backup_references.py
chmod +x scripts/utils/validation/verify_backup_system.sh
chmod +x scripts/utils/backup/create_backup_branch.sh
chmod +x scripts/utils/backup/restore_from_backup.sh
...
./scripts/utils/cleanup/remove_local_backup_system.sh
...
./scripts/utils/validation/verify_backup_system.sh
...
echo -e "  ./scripts/utils/backup/create_backup_branch.sh"

echo -e "\n${BLUE}To restore from a backup:${NC}"
echo -e "  ./scripts/utils/backup/restore_from_backup.sh [backup-branch-name] [file1 file2 ...]"
